...
The one-click start is a developer-triggered, one-shot service check for the
Ubuntu ThirdHand host. It keeps expected services that are already listening,
starts only missing services in profile order, prints one result per service,
and then exits.
...
|---|---|---|
| Robot Service | `127.0.0.1:3000` | Keep when listening; otherwise start it. Never connect CAN automatically. |
...
Port `3200` is retired and is not part of this workflow. The tool checks service
ports and lightweight identity only. It does not repair models, cameras,
dependencies, CAN state, or application-level debugging failures.
...
2. existing service ports are kept unchanged;
3. missing services are started sequentially;
...
- restart an already listening expected service;
