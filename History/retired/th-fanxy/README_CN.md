...
| Lumos HTTP | `/health`、`/frame.jpg`、`/frame_raw.jpg`、`/camera_lumos` | OpenCV/V4L2 | 相机只读；预览与按需原始帧携带同源序号和 monotonic header；[服务](web-control/server/lumos_http_server.py) |
...
| Lumos HTTP | `0.0.0.0:3001`；`/health`、`/frame.jpg`、`/frame_raw.jpg`、`/camera_lumos` | 无认证 | camera-only、只读；预览/原始 frame 含 sequence/monotonic headers，不获机器人权限 | Ctrl+C/进程停止仅结束相机服务 | L3 只读；按教程改绑 loopback |
...
| --- | --- | --- | --- | --- | --- |
| V1 | Lumos 实例 mask、D435 depth 和机器人基座坐标系 3D 视图 <code>content:v1:mask-depth-base3d</code> | Planned Evidence | 带时间戳的 Lumos RGB、D435 depth、calibration ID/hash、实例 masks、registered 3D points、experiment manifest <code>source:v1:rgb-depth-calibration-mask-points</code> | 版本化 script/command 按 manifest 对齐帧、应用记录的 calibration 并渲染已选样本 <code>generator:v1:manifest-calibrated-panel</code> | manifest 和命令无需手工改图即可复现 panel <code>gate:v1:reproducible-no-manual-edit</code> |
| V2 | 遮挡前、遮挡中和重捕获后的身份 <code>content:v2:occlusion-reacquisition-identity</code> | Planned Evidence | 有序 replay frames、detections/masks、track IDs、association scores、memory state、occlusion annotations、experiment manifest <code>source:v2:replay-tracks-memory-annotations</code> | 版本化 script/command 选择声明的 clips，并从 result records 渲染三个状态 <code>generator:v2:declared-clips-three-states</code> | clip split 为 held-out，且 identity labels/selection rule 已记录 <code>gate:v2:heldout-identity-selection</code> |
| V3 | 深度注册误差、径向标定残差和不确定度视图 <code>content:v3:registration-residual-uncertainty</code> | Planned Evidence | calibration targets、correspondence/residual records、radial-bin metadata、registration errors、covariance/uncertainty records、manifest <code>source:v3:calibration-correspondence-covariance</code> | 版本化 script/command 将 source JSON/CSV 聚合为残差、误差和不确定度 panels <code>generator:v3:aggregate-residual-error-uncertainty</code> | 单位、聚合、calibration ID 和排除样本均已报告 <code>gate:v3:units-aggregation-calibration-exclusions</code> |
| L1 | VLA instruction、visual context、candidate、preview、confirmation 和 refusal reason <code>content:l1:instruction-candidate-preview-refusal</code> | Planned Evidence | 去标识 scenario input、visual-context reference、structured candidate、preview output、confirmation event、validator/refusal log、manifest <code>source:l1:scenario-context-candidate-validator-log</code> | 版本化 script/command 从 logs 渲染完整可审计 decision trace <code>generator:l1:auditable-decision-trace</code> | trace 不含 secret、个人信息或误导性的 actuator-success claim <code>gate:l1:no-secrets-or-actuator-success-claim</code> |
| E1 | baseline comparison 和 confidence intervals <code>content:e1:baseline-confidence-intervals</code> | Planned Evidence | 所有预注册 baselines 的 result records、sample counts、intervals、slices、source CSV/JSON、manifests <code>source:e1:preregistered-results-intervals-slices</code> | 版本化 script/command 读取 result records 并计算/绘制声明的 aggregate 和 confidence intervals <code>generator:e1:aggregate-confidence-interval-plot</code> | baselines、split、interval method 和 exclusions 与矩阵一致 <code>gate:e1:baseline-split-interval-exclusions</code> |
| E2 | ablation table/curve <code>content:e2:ablation-table-curve</code> | Planned Evidence | controlled-variant result records、variant configuration、seeds、slices、source CSV/JSON、manifests <code>source:e2:controlled-variants-seeds-results</code> | 版本化 script/command 按 variants 分组，生成声明的 table 或 curve <code>generator:e2:grouped-variant-table-curve</code> | one-delta ablation rule 和所有 controls 均有文档记录 <code>gate:e2:one-delta-controls</code> |
| E3 | accuracy-latency-resource trade-off <code>content:e3:accuracy-latency-resource</code> | Planned Evidence | accuracy、P50/P95 latency、FPS、CPU/GPU/VRAM records、hardware/environment manifests、source CSV/JSON <code>source:e3:metrics-environment-manifest</code> | 版本化 script/command 按 experiment ID 连接 metrics 并渲染 trade-off points/error bars <code>generator:e3:experiment-metric-tradeoff</code> | hardware、batch/input settings 和 aggregation window 可比较 <code>gate:e3:comparable-hardware-window</code> |
| F1 | representative success/failure cases <code>content:f1:representative-success-failure</code> | Planned Evidence | 声明的 case-selection rule、scenario/clip IDs、input/output traces、failure taxonomy、manifests <code>source:f1:selection-traces-failure-taxonomy</code> | 版本化 script/command 应用 selection rule 并渲染成对 success/failure cases <code>generator:f1:selection-rule-paired-cases</code> | 不可 cherry-pick；limitations 和 refusal/failure context 必须可见 <code>gate:f1:no-cherry-pick-limitations-visible</code> |
