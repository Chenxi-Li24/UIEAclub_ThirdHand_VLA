...
def test_grasp_move_keeps_the_authorized_startouch_tcp_target(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The SDK pose is already expressed at its configured gripper TCP."""
    bridge = object.__new__(startouch_bridge.RobotBridge)
    bridge.arm_lock = threading.RLock()
    bridge.connected = True
    bridge.state_ready = True
    ...
    target = [0.5090842473659607, 0.028378244701246277, 0.08008813438899774]
    ...
    bridge.move_linear(
        {
            ...
            "euler": HOME_EULER.tolist(),
            "time_sec": 8.0,
            "request_id": "grasp-pregrasp-1",
            "source": "grasp:physical_validation_pregrasp",
        }
    )

    queued = bridge.motion_queue.get_nowait()
    assert queued["_move_l_pos"] == pytest.approx(target)
    assert queued["_authorized_tcp_pos"] == pytest.approx(target)
    assert (
        "command_accepted",
        {
            "command": "move_l",
