...
def test_pairer_carries_synchronized_d435_rgb_without_changing_canonical_role() -> None:
    roles = CameraRoleMap("lumos_rgb", "d435_depth", "d435_rgb", "cross_camera")
    lumos = RgbFrame(
        FrameStamp("lumos_rgb", 4, 1_000_000_000),
        np.zeros((4, 4, 3), dtype=np.uint8),
    )
    d435_rgb = RgbFrame(
        FrameStamp("d435_rgb", 9, 1_010_000_000),
        np.ones((3, 5, 3), dtype=np.uint8),
    )
    depth = DepthFrame(
        FrameStamp("d435_depth", 9, 1_010_000_000),
    ...
    assert pair.rgb is lumos
    assert pair.debug_rgb is d435_rgb
    assert pair.depth is depth
    assert pair.reasons == ()


def test_pairer_rejects_d435_rgb_not_synchronized_with_its_depth() -> None:
    roles = CameraRoleMap("lumos_rgb", "d435_depth", "d435_rgb", "cross_camera")
    lumos = RgbFrame(
        FrameStamp("lumos_rgb", 4, 1_000_000_000),
        np.zeros((4, 4, 3), dtype=np.uint8),
    ...
    d435_rgb = RgbFrame(
        FrameStamp("d435_rgb", 8, 1_010_000_000),
        np.ones((3, 5, 3), dtype=np.uint8),
    )
    depth = DepthFrame(
        FrameStamp("d435_depth", 9, 1_010_000_000),
    ...
    with pytest.raises(InvalidDataError, match="synchronized"):
        LatestFramePairer(roles, 50_000_000, 200_000_000).pair(
            lumos,
            depth,
            1_020_000_000,
            debug_rgb=d435_rgb,
