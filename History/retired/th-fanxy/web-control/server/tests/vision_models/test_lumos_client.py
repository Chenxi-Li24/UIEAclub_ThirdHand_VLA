...
def test_client_accepts_new_epoch_when_sequence_resets_and_monotonic_time_advances() -> None:
    body = jpeg_bytes()
    headers = {
        "Content-Type": "image/jpeg",
        "X-Lumos-Sequence": "100",
        "X-Lumos-Monotonic-Ns": "123000000",
    }
    server, thread = serve(headers, body)
    try:
        client = LumosSnapshotClient(
            f"http://127.0.0.1:{server.server_port}/frame.jpg",
            timeout_s=1.0,
        )
        first = client.read(after_sequence=99)
        assert first is not None
        headers["X-Lumos-Sequence"] = "2"
        headers["X-Lumos-Monotonic-Ns"] = "124000000"

        restarted = client.read(after_sequence=100)

        assert restarted is not None
        assert restarted.stamp == FrameStamp("lumos_rgb", 2, 124_000_000)
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=2.0)
...
def test_client_accepts_native_raw_snapshot_endpoint() -> None:
    client = LumosSnapshotClient(
        "http://127.0.0.1:3001/frame_raw.jpg",
        timeout_s=1.0,
    )
    assert client.url == "http://127.0.0.1:3001/frame_raw.jpg"
