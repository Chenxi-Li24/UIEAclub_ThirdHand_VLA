...
class ActiveViewSession:
    ...
    def transition(
        ...
        if self.phase is ActiveViewPhase.GRASP_PREVIEW and isinstance(
            event, GraspPreviewReady
        ):
            return replace(
                self,
                phase=ActiveViewPhase.COMPLETE,
                updated_ns=event.observed_ns,
                proposal=None,
                request_id=None,
            )
    ...
    def _on_initial_proposal(self, event: ProposalReady) -> "ActiveViewSession":
        ...
        if proposal.kind == "refine_delta":
            # The target may already have valid D435 depth while being outside
            # the central ROI. In that case a bounded lateral correction is
            # the correct first action; forcing an unnecessary coarse pose
            # discards the available metric observation.
            return replace(
                self,
        ...
        if proposal.kind != "coarse_pose":
            raise InvalidTransition("initial active-view proposal kind is invalid")
        return replace(
            self,
    ...
    def _on_move_started(self, event: MoveStarted) -> "ActiveViewSession":
        ...
        # Expiry is consumed by OperatorConfirmed.  Once the evidence-bound
        # proposal is authorized, a later correlated MoveStarted belongs to
        # that authorization and is governed by the execution timeout instead
        # of the original perception proposal TTL.
...
class ActiveViewSessionCoordinator:
    ...
    def refinement_evidence_ids(self) -> tuple[str, ...]:
        """Evidence for a refinement chained from this session's completed move."""

        session = self.session
        if (
            session is None
            or session.phase in {ActiveViewPhase.ABORTED, ActiveViewPhase.COMPLETE}
            or session.refinement_steps < 1
            or session.proposal is None
            or session.proposal.kind != "refine_delta"
        ):
            return ()
        return session.proposal.evidence_ids
    ...
    def observe_frame(
        ...
        *,
        observed_ns: int,
        arm_stationary: bool,
        arm_observed_ns: int | None,
        identity_id: int | None,
        identity_confirmed: bool,
        pose: PoseEstimate | None,
        depth_quality: DepthQuality | None,
        grasp_preview_allowed: bool,
    ) -> list[dict[str, Any]]:
        """Advance one post-motion lifecycle step from trusted in-process data."""

        observed = _timestamp(observed_ns)
        if not isinstance(arm_stationary, bool):
            raise InvalidDataError("active-view frame stationarity must be explicit")
        if not isinstance(identity_confirmed, bool) or not isinstance(
            grasp_preview_allowed, bool
        ):
            raise InvalidDataError("active-view frame gates must be explicit")
        session = self.session
        if session is None or session.phase in {
            ...
            return []
        if observed <= session.updated_ns:
            return []

        if session.phase is ActiveViewPhase.SETTLING:
            if not arm_stationary or arm_observed_ns is None:
                return []
            arm_observed = _timestamp(arm_observed_ns)
            if arm_observed <= session.updated_ns:
                return []
            self.session = session.transition(
                Settled(session.session_id, arm_observed),
                self.config,
            )
        elif session.phase is ActiveViewPhase.VERIFYING_IDENTITY:
        ...
        elif session.phase is ActiveViewPhase.ACQUIRING_DEPTH:
            if identity_id is None:
                return []
            if identity_id != session.identity_id or not identity_confirmed:
                self.session = session.transition(
                    IdentityObserved(
                        session.session_id,
                        identity_id,
            ...
            elif pose is None or depth_quality is None:
                return []
            elif pose.stamp.monotonic_ns < session.updated_ns:
                return []
            else:
                self.session = session.transition(
                    DepthObserved(
        ...
        elif session.phase is ActiveViewPhase.GRASP_PREVIEW:
            if not grasp_preview_allowed:
                return []
            self.session = session.transition(
                GraspPreviewReady(session.session_id, observed),
                self.config,
            )
        else:
            return []
        ...
        *,
        ...
        try:
            if command_type == "active_view_operator_confirmed":
                ...
                if self.session.phase is ActiveViewPhase.MOVE_AUTHORIZED:
                    return [
                        {
