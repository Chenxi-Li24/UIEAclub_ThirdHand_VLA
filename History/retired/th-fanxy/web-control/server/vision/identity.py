...
class PersistentIdentityMemory:
    ...
    def update(
        ...
        for row, column in matches:
            ...
            if observation.pose is None:
                # Lumos may publish between D435 frames. An RGB-only update is
                # not evidence that the latest metric-pose chain was broken;
                # the next RGB-D sample still has to pass its bounded age and
                # position gates.
                pass
