...
const HARD_MAX_REFINEMENT_STEPS = 6;
const HARD_MAX_TOTAL_TRANSLATION_M = 0.060;
const HARD_MAX_TOTAL_ROTATION_RAD = 15 * Math.PI / 180;
...
function authorizeActiveViewMove({ requested, approval, proposal, robot, session, nowMs, limits }) {
  ...
  if (safeLimits.maxTranslationM * safeLimits.maxRefinementSteps >
      HARD_MAX_TOTAL_TRANSLATION_M + 1e-12) {
    return reject('total_translation_budget_exceeds_hard_limit');
  }
  if (safeLimits.maxRotationRad * safeLimits.maxRefinementSteps >
      HARD_MAX_TOTAL_ROTATION_RAD + 1e-12) {
    return reject('total_rotation_budget_exceeds_hard_limit');
  }
  ...
  if (proposal.kind === 'refine_delta') {
    ...
    const rotation = Math.hypot(...proposal.rotationDeltaRad);
    if (translation <= 1e-12 && rotation <= 1e-12) {
      return reject('refinement_translation_too_small');
    }
    ...
    if (rotation > safeLimits.maxRotationRad + 1e-12) {
      return reject('refinement_rotation_exceeds_limit');
    }
    return {
      approved: true,
      reason: 'authorized',
