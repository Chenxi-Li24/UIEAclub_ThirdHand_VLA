...
stdout carries D435 debug MJPEG, fd 3 carries JSON events, fd 4 carries the
canonical Lumos perception overlay, and fd 5 carries raw D435 RGB for offline
calibration capture. This process never opens robot/CAN.
"""
...
from vision.online_frames import DepthFrame, LatestFramePairer, RgbFrame
...
from vision_models.grasp_online import (
    OnlineGraspPreviewAdapter,
    load_grasp_preview_config,
)
from vision_models.active_view_catalog import (
    ActiveViewEvidenceError,
    ActiveViewEvidenceGuard,
    ActiveViewFoundationGuard,
)
...
GRASP_PREVIEW_CONFIG = Path(
    os.environ.get(
        "GRASP_PREVIEW_CONFIG",
        str(Path(__file__).resolve().parents[2] / "configs/vision/grasp_preview.yaml"),
    )
).resolve()
...
ACTIVE_VIEW_CATALOG = Path(
    os.environ.get(
        "ACTIVE_VIEW_CATALOG",
        str(ACTIVE_VIEW_EVIDENCE_DIR / "observation-catalog.json"),
    )
).resolve()
...
D435_RAW_FD = int(os.environ.get("D435_RAW_FD", "5"))
ARM_STATIONARY_MAX_VELOCITY_DEG_S = 2.0
...
_d435_raw_output_lock = threading.Lock()
_overlay_lock = threading.Lock()
shutdown_flag = threading.Event()


@dataclass(frozen=True)
class RuntimeActiveViewEvidence:
    foundation_guard: Optional[ActiveViewFoundationGuard]
    motion_guard: Optional[ActiveViewEvidenceGuard]
    blockers: tuple[str, ...]
    detail: Optional[str]


def load_runtime_active_view_evidence() -> RuntimeActiveViewEvidence:
    """Load 3D foundation independently from the motion-pose catalog."""

    try:
        foundation_guard = ActiveViewFoundationGuard.load(
            ACTIVE_VIEW_CAMERA_EVIDENCE,
            ACTIVE_VIEW_TABLE_EVIDENCE,
            evidence_dir=ACTIVE_VIEW_EVIDENCE_DIR,
        )
    except (ActiveViewEvidenceError, OSError, ValueError) as error:
        return RuntimeActiveViewEvidence(
            None,
            None,
    ...
    try:
        motion_guard = ActiveViewEvidenceGuard.load(
            ACTIVE_VIEW_CAMERA_EVIDENCE,
            ACTIVE_VIEW_TABLE_EVIDENCE,
            ACTIVE_VIEW_CATALOG,
            evidence_dir=ACTIVE_VIEW_EVIDENCE_DIR,
        ...
        return RuntimeActiveViewEvidence(
            foundation_guard,
            None,
            ("active_view_catalog_unavailable",),
            str(error)[:512],
        )
    return RuntimeActiveViewEvidence(foundation_guard, motion_guard, (), None)


def bind_active_view_evidence(active_view_config, runtime: RuntimeActiveViewEvidence):
    """Bind foundation geometry and motion poses through separate gates."""

    if runtime.foundation_guard is None:
        return active_view_config
    poses = ()
    if runtime.motion_guard is not None:
        poses = runtime.motion_guard.evidence.poses
    return replace(
        active_view_config,
        table_plane=runtime.foundation_guard.evidence.table,
        observation_poses=poses,
    )


def build_online_grasp_preview(
    runtime: RuntimeActiveViewEvidence,
) -> Optional[OnlineGraspPreviewAdapter]:
    """Compose preview geometry only when camera/table evidence is available."""

    if runtime.foundation_guard is None:
        return None
    foundation = runtime.foundation_guard.evidence
    return OnlineGraspPreviewAdapter(
        table=foundation.table,
        config=load_grasp_preview_config(GRASP_PREVIEW_CONFIG),
        foundation_evidence_ids=(
            foundation.camera_source_id,
            foundation.table_source_id,
        ),
    )


def _configure_outputs() -> None:
    global _d435_raw_file, _event_file, _overlay_file
    _event_file = os.fdopen(EVENT_FD, "w", buffering=1, encoding="utf-8", closefd=False)
    _d435_raw_file = os.fdopen(D435_RAW_FD, "wb", buffering=0, closefd=False)
...
def _write_mjpeg(
    ...
    *,
    ...
    if provenance is not None:
        frame_id = int(provenance["frame_id"])
        monotonic_ns = int(provenance["monotonic_ns"])
        observed_at_ms = int(provenance["ts"])
        if min(frame_id, monotonic_ns, observed_at_ms) < 0:
            raise ValueError("overlay provenance values must be non-negative")
        digest = hashlib.sha256(jpeg).hexdigest()
        headers.extend(
            [
                f"X-Thirdhand-Frame-Id: {frame_id}".encode("ascii"),
                f"X-Thirdhand-Monotonic-Ns: {monotonic_ns}".encode("ascii"),
                f"X-Thirdhand-Observed-At-Ms: {observed_at_ms}".encode("ascii"),
                f"X-Thirdhand-Image-Sha256: sha256:{digest}".encode("ascii"),
            ]
        )
    part = b"\r\n".join(headers) + b"\r\n\r\n" + jpeg + b"\r\n"
    with lock:
...
class TimestampedHistoryBuffer(Generic[T]):
    """Small ordered history used to pair robot state with captured frames."""

    def __init__(self, *, capacity: int = 16) -> None:
        if isinstance(capacity, bool) or not isinstance(capacity, int) or not 2 <= capacity <= 256:
            raise ValueError("timestamped history capacity must be within [2, 256]")
        self.capacity = capacity
        self._lock = threading.Lock()
        self._sequence = 0
        self._values: deque[tuple[int, int, T]] = deque(maxlen=capacity)

    def publish(self, value: T, *, monotonic_ns: int) -> int:
        if isinstance(monotonic_ns, bool) or not isinstance(monotonic_ns, int) or monotonic_ns <= 0:
            raise ValueError("timestamped history time must be a positive integer")
        with self._lock:
            if self._values and monotonic_ns <= self._values[-1][1]:
                raise ValueError("timestamped history time must strictly advance")
            self._sequence += 1
            self._values.append((self._sequence, monotonic_ns, value))
            return self._sequence
    ...
    def closest(
        ...
        *,
        ...
        if isinstance(monotonic_ns, bool) or not isinstance(monotonic_ns, int) or monotonic_ns <= 0:
            raise ValueError("timestamped history query time must be a positive integer")
        if (
            isinstance(after_sequence, bool)
            or not isinstance(after_sequence, int)
            or after_sequence < 0
        ):
            raise ValueError("timestamped history sequence must be non-negative")
        with self._lock:
            candidates = [item for item in self._values if item[0] > after_sequence]
            if not candidates:
                return None
            sequence, _stamp, value = min(
                candidates,
                key=lambda item: (
                    abs(item[1] - monotonic_ns),
                    item[1] > monotonic_ns,
                    -item[0],
                ),
            )
...
def parse_arm_state(
    ...
    *,
    ...
    if stationary and np.any(
        np.abs(velocities) > ARM_STATIONARY_MAX_VELOCITY_DEG_S
    ):
...
d435_history: TimestampedHistoryBuffer[D435Sample] = TimestampedHistoryBuffer(
    capacity=32
)
arm_pose_history: TimestampedHistoryBuffer[ArmPoseSample] = TimestampedHistoryBuffer(
    capacity=16
)
...
def _capture_d435_once(provenance: FrameStampSequencer) -> None:
    ...
    try:
        ...
        emit(
            ...
            d435_ready=True,
            d435_serial=serial,
            d435_rgb_role="same_instance_verification_and_debug",
        ...
        while not shutdown_flag.is_set():
            ...
            if not raw_ok:
                raise RuntimeError("D435 raw JPEG encoding failed")
            ...
            sample = D435Sample(
                ...
                rgb=RgbFrame(
                    FrameStamp("d435_rgb", stamp.frame_id, stamp.monotonic_ns),
            ...
            sequence = d435_history.publish(
                sample,
                monotonic_ns=stamp.monotonic_ns,
            )
            _update_state(d435_sequence=sequence)
            _write_mjpeg(_d435_raw_file, raw_encoded.tobytes(), _d435_raw_output_lock)
...
def _load_online_engine():
    ...
    runtime_evidence = load_runtime_active_view_evidence()
    active_view_config = bind_active_view_evidence(active_view_config, runtime_evidence)
    emit(
        "vision_status",
        online=False,
    ...
    grasp_preview = build_online_grasp_preview(runtime_evidence)
    d435_instance = OnlineD435InstanceAdapter(
        segmenter,
        D435InstanceVerifier(
            D435InstanceVerifierConfig(
                min_projected_points=40,
    ...
    return config, OnlinePerceptionEngine(
        ...
        d435_instance_verifier=d435_instance,
    ), active_view_config, runtime_evidence
...
def run_online_perception() -> None:
    ...
    try:
        config, engine, active_view_config, runtime_evidence = _load_online_engine()
    ...
    coordinator = None
    if runtime_evidence.motion_guard is not None:
        motion_guard = runtime_evidence.motion_guard
        coordinator = ActiveViewSessionCoordinator(
            active_view_config,
            evidence_ids=(
                motion_guard.evidence.evidence_id,
                motion_guard.evidence.camera.calibration.calibration_id,
            ),
        )
    ...
    while not shutdown_flag.is_set():
        try:
            ...
            d435_item = d435_history.closest(
                rgb.stamp.monotonic_ns,
                after_sequence=last_d435_sequence,
            )
            depth = None
            d435_rgb = None
            ...
            result = engine.process(
                ...
                refinement_evidence_ids=(
                    ()
                    if coordinator is None
                    else coordinator.refinement_evidence_ids()
                ),
            )
            if coordinator is not None and coordinator.session is not None:
                active_identity = coordinator.session.identity_id
                matching_target = next(
                    (
                        target
                        for target in result.targets
                        if target.identity_id == active_identity
                    ),
                    None,
                )
                matching_observation = next(
                    (
                        observation
                        for observation in result.active_view_observations
                        if observation.identity_id == active_identity
                    ),
                    None,
                )
                for active_event in coordinator.observe_frame(
                    observed_ns=now_ns,
                    arm_stationary=arm_stationary,
                    arm_observed_ns=(
                        None if arm is None else arm.stamp.monotonic_ns
                    ),
                    identity_id=(
                        None
                        if matching_target is None
                        else matching_target.identity_id
                    ),
                    identity_confirmed=(
                        matching_target is not None
                        and matching_target.identity_status.value == "confirmed"
                    ),
                    pose=None if matching_target is None else matching_target.pose,
                    depth_quality=(
                        None
                        if matching_observation is None
                        else matching_observation.quality
                    ),
                    grasp_preview_allowed=bool(
                        matching_target is not None
                        and matching_target.grasp_preview is not None
                        and matching_target.grasp_preview.allowed
                    ),
                ):
                    emit_event(active_event)
            if (
            ...
            blockers.extend(runtime_evidence.blockers)
...
def _handle_arm_state(command: dict[str, Any]) -> None:
    ...
    try:
        ...
        return
    sequence = arm_pose_history.publish(
        sample,
        monotonic_ns=sample.stamp.monotonic_ns,
    )
    _update_state(
