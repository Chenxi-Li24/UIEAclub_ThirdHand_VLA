...
def test_depth_quality_uses_only_pixels_inside_verified_d435_instance() -> None:
    registered = registered_cloud(point_grid())
    ...
    quality = evaluate_depth_quality(
        ...
        t_base_from_lumos=np.eye(4),
        config=config(),
        d435_instance_mask=d435_mask,
    )

    assert 0 < quality.valid_points < 100
    assert "insufficient_depth_points" in quality.reasons


def test_off_center_depth_produces_clipped_lateral_and_optical_refinement() -> None:
    ...
    assert np.linalg.norm(proposal.delta_base_m) == pytest.approx(0.010)
    ...
    assert np.linalg.norm(proposal.rotation_delta_rad) == pytest.approx(
        np.deg2rad(2.45)
    )
    assert proposal.rotation_delta_rad[0] == pytest.approx(0.0)
    assert proposal.rotation_delta_rad[1] > 0.0
    assert proposal.rotation_delta_rad[2] == pytest.approx(0.0)


def test_refinement_rotation_stays_below_execution_limit_after_transform_roundoff() -> None:
    registered = registered_cloud(point_grid(center_x=0.35))
    quality = evaluate_depth_quality(
        registered,
        np.ones(registered.valid.shape, dtype=bool),
    ...
    nearly_rigid_transform = np.eye(4)
    nearly_rigid_transform[1, 1] = 1.0 + 4e-9

    proposal = propose_refinement(
        3,
        quality,
        nearly_rigid_transform,
        FrameStamp("lumos+d435", 2, 1_000),
        1_000,
        0,
        config(),
    ...
    assert proposal.kind == "refine_delta"
    assert np.linalg.norm(proposal.rotation_delta_rad) < np.deg2rad(2.5)


def test_tilted_tabletop_refinement_never_lowers_the_tcp() -> None:
    registered = registered_cloud(point_grid(center_x=0.35))
    quality = evaluate_depth_quality(
        registered,
        np.ones(registered.valid.shape, dtype=bool),
    ...
    tilted_camera = np.eye(4)
    tilted_camera[:3, :3] = np.array(
        [
    ...
    assert proposal.kind == "refine_delta"
    np.testing.assert_allclose(proposal.delta_base_m, np.zeros(3), atol=1e-12)
    assert np.linalg.norm(proposal.rotation_delta_rad) > 0.0
    assert np.dot(proposal.delta_base_m, config().table_plane.normal_base) == pytest.approx(0.0)
    assert np.dot(proposal.delta_base_m, proposal.optical_axis_base) == pytest.approx(0.0)
