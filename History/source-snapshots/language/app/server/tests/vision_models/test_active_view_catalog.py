...
def test_foundation_guard_remains_usable_without_observation_catalog(tmp_path: Path) -> None:
    evidence_dir = tmp_path / "evidence"
    camera_path, table_path, catalog_path = write_evidence(evidence_dir)
    catalog_path.unlink()

    guard = ActiveViewFoundationGuard.load(
        camera_path,
        table_path,
        evidence_dir=evidence_dir,
    )

    assert guard.evidence.camera.calibration.validated is True
    assert guard.evidence.table.validated is True
    assert guard.verify_unchanged() is True

    table = json.loads(table_path.read_text(encoding="utf-8"))
    table.pop("content_id")
    table["position_rmse_m"] = 0.005
    table_path.write_text(json.dumps(addressed(table)), encoding="utf-8")
