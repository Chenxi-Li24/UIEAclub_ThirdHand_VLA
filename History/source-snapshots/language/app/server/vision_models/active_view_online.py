...
class ActiveViewConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if max_refinement_steps > 6:
            raise InvalidDataError("active-view refinement limit cannot exceed 6 steps")
        if max_translation_m * max_refinement_steps > 0.060 + 1e-12:
            raise InvalidDataError("active-view total translation cannot exceed 60 mm")
        if max_rotation_rad * max_refinement_steps > float(np.deg2rad(15.0)) + 1e-12:
            raise InvalidDataError("active-view total rotation cannot exceed 15 degrees")
...
class ActiveViewTargetReport:
    ...
    def __post_init__(self) -> None:
        ...
        if self.remaining_refinements > 6:
            raise InvalidDataError("active-view report refinements cannot exceed six")
...
@dataclass(frozen=True)
class ActiveViewTargetObservation:
    """Trusted in-process depth observation; never serialized to the browser."""
    ...
    def __post_init__(self) -> None:
        for name in ("detection_id", "identity_id"):
            value = getattr(self, name)
            if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                raise InvalidDataError(f"active-view {name} must be non-negative")
        if not isinstance(self.quality, DepthQuality):
            raise InvalidDataError("active-view observation requires depth quality")
...
class ActiveViewEvaluationBatch:
    ...
    observations: tuple[ActiveViewTargetObservation, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if len(observations) > len(reports) or any(
            not isinstance(observation, ActiveViewTargetObservation)
            for observation in observations
        ):
            raise InvalidDataError("active-view observation batch is invalid or unbounded")
...
class ActiveViewDryRunAdapter:
    ...
    def evaluate_with_proposals(
        ...
        *,
        ...
        if fusion_context is not None and not isinstance(
            fusion_context, DualCameraFusionContext
        ):
            raise InvalidDataError("active-view fusion context is invalid")
        continued_evidence = validated_evidence_ids(
            refinement_evidence_ids,
            allow_empty=True,
        )
        ...
        observations: list[ActiveViewTargetObservation] = []
        for detection in detection_items[: self.max_reports]:
            ...
            verification = (
                ...
                else d435_verifications.get(detection.detection_id)
            )
            if d435_verifications is not None and (
                ...
                reasons.extend(
                    ...
                    else verification.blockers
                )
            if (
                fusion_context is not None
                and calibration is not None
                and target.identity_id is not None
                and arm_stationary
                and (d435_verifications is None or verification is not None and verification.verified)
                and fusion_context.calibration_id
                == calibration.calibration.calibration_id
            ):
                try:
                    quality = evaluate_depth_quality(
                        fusion_context.registered,
                        detection.mask,
                        calibration.d435,
                        fusion_context.t_d435_from_lumos,
                        fusion_context.t_base_from_lumos,
                        self.config,
                        d435_instance_mask=(
                    ...
                    reasons.append("depth_quality_unavailable")
            if quality is not None and target.identity_id is not None:
                observations.append(
                ...
                try:
                    ...
                    if self.config.observation_poses:
                        if current_joints_deg is None:
                            ...
                            quality is not None
                            and quality.valid_points >= self.config.min_depth_points
                            and not quality.acceptable
                            and quality.center_d435_m is not None
                        ):
                            current_pose_id = match_observation_pose(
                                current_joints_deg,
                                self.config.observation_poses,
                            )
                            if current_pose_id is None and not continued_evidence:
                                reasons.append("current_pose_unvalidated")
                            else:
                                proposal_evidence = continued_evidence
                                if current_pose_id is not None:
                                    current_pose = next(
                                        pose
                                        for pose in self.config.observation_poses
                                        if pose.pose_id == current_pose_id
                                    )
                                    proposal_evidence = tuple(
                                        dict.fromkeys(
                                            (
                                                *fusion_context.evidence_ids,
                                                calibration.calibration.calibration_id,
                                                current_pose.path_validation_id,
                                            )
                                        )
                                    )
                                t_base_from_d435 = validate_transform(
                                    fusion_context.t_base_from_lumos
                                    @ np.linalg.inv(fusion_context.t_d435_from_lumos)
                                )
                        ...
                        elif quality is not None and quality.acceptable:
                            pass
                        else:
            ...
            reports.append(
                ActiveViewTargetReport(
                    ...
                    central_fraction=None if quality is None else quality.central_fraction,
        ...
        return ActiveViewEvaluationBatch(
            tuple(reports),
            tuple(proposals),
            tuple(observations),
        )
...
def load_active_view_config_dict(raw: Any) -> ActiveViewConfig:
    ...
    if max_translation_m * max_refinement_steps > 0.060 + 1e-12:
        raise InvalidDataError("active-view total translation cannot exceed 60 mm")
    if np.deg2rad(max_rotation_deg) * max_refinement_steps > np.deg2rad(15.0) + 1e-12:
        raise InvalidDataError("active-view total rotation cannot exceed 15 degrees")
