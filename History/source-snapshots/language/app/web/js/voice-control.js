...
const DEFAULT_ENDPOINT = `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.hostname}:3001/v1/voice`;
const ENDPOINT_MIGRATION_KEY = 'voiceAiEndpointUbuntuPcV1';
  ...
  async init() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (/^wss?:\/\//i.test(runtimeConfig?.language?.voiceEndpoint || '')) {
          this.runtimeVoiceEndpoint = runtimeConfig.language.voiceEndpoint;
        }
