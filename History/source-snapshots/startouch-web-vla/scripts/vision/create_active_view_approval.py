...
def _validated_limits(raw: Mapping[str, object]) -> dict[str, object]:
    ...
    if isinstance(refinements, bool) or not isinstance(refinements, int) or not 1 <= refinements <= 3:
        raise ValueError("refinement limit must be between one and three")
