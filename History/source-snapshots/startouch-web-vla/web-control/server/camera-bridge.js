...
class CameraBridge extends EventEmitter {
  ...
  buildSpawnSpec() {
    ...
    const requestedScript = this.config.script || process.env.CAMERA_BRIDGE_SCRIPT ||
      'camera_bridge.py';
    const scriptName = path.basename(requestedScript);
    const supportedScripts = new Set([
      'camera_bridge.py',
      'camera_bridge_xvisio.py',
      'camera_bridge_va.py',
    ]);
    if (!supportedScripts.has(scriptName)) {
      throw new Error('Unsupported camera bridge script');
    }
    const script = path.isAbsolute(requestedScript)
      ? requestedScript
      : path.join(__dirname, scriptName);
    const xvisioStreamExecutable = this.config.xvisioStreamExecutable ||
      process.env.XVISIO_STREAM_EXECUTABLE || '';
    const usesExtendedStreams = scriptName === 'camera_bridge_va.py';
