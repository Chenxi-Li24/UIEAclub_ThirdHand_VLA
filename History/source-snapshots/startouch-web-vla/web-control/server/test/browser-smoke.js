...
const DEFAULT_VOICE_ENDPOINT = 'ws://127.0.0.1:3002/v1/voice';
...
async function waitForDebugTarget() {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    try {
      const response = await fetchCompat(`http://127.0.0.1:${DEBUG_PORT}/json`);
...
async function run() {
  ...
  const defaultRouteState = await evaluate(`(() => ({
    ...
    gpuPressed: document.querySelector(
      '[data-voice-port="3002"]'
    )?.getAttribute('aria-pressed'),
    cpuPressed: document.querySelector(
      '[data-voice-port="3001"]'
    )?.getAttribute('aria-pressed'),
  ...
  const visionUi = await evaluate(`(() => {
    document.querySelector('[data-xvision-stream="raw"]').click();
    document.querySelector('[data-grasp-mode="auto"]').click();
    return {
      oldD435Absent: !document.getElementById('sec-d435') &&
        !document.getElementById('camera-feed'),
      feedUsesProxy: document.getElementById('xvision-feed').src.includes('/camera/xvisio/raw'),
      recognitionModeAvailable: Boolean(document.querySelector('[data-xvision-stream="vision"]')),
      autoSelected: document.querySelector('[data-grasp-mode="auto"]')
        .getAttribute('aria-pressed') === 'true',
      graspControlsPresent: ['btn-grasp-start', 'btn-grasp-next', 'btn-grasp-cancel']
        .every(id => Boolean(document.getElementById(id))),
      graspLockedByDefault: document.getElementById('btn-grasp-start').disabled,
    };
  })()`);
  ...
  const checks = {
    defaultLocalEndpoint: defaultEndpoint === DEFAULT_VOICE_ENDPOINT,
    ...
    xvisionReplacesD435Panel: visionUi.oldD435Absent,
    xvisionDualStreamsAvailable:
      visionUi.feedUsesProxy && visionUi.recognitionModeAvailable,
    stagedAndAutoGraspControls:
      visionUi.autoSelected && visionUi.graspControlsPresent,
    visionGraspFailsClosed: visionUi.graspLockedByDefault,
