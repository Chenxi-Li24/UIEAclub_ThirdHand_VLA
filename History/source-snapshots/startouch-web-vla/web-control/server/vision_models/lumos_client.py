...
class LumosSnapshotClient:
    def __init__(
        ...
        *,
        ...
        if parsed.scheme != "http" or not parsed.hostname or parsed.path != "/frame.jpg":
            raise ModelContractError("Lumos snapshot URL must be an HTTP /frame.jpg endpoint")
    ...
    def read(self, after_sequence: int) -> RgbFrame | None:
        ...
        try:
            response = urllib.request.urlopen(self.url, timeout=self.timeout_s)
