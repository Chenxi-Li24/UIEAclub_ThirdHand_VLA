...
const excessiveTotalBudget = { ...limits, maxTranslationM: 0.020 };
assert.deepEqual(
  decide({ limits: excessiveTotalBudget, approval: makeApproval({
    limits: { ...makeApproval().limits, max_translation_m: 0.020 },
  }) }),
  { approved: false, reason: 'total_translation_budget_exceeds_hard_limit' }
);
...
assert.deepEqual(
  decide({ proposal: refinement({ deltaBaseM: [0.0101, 0, 0] }) }),
  { approved: false, reason: 'refinement_translation_exceeds_limit' }
);
assert.deepEqual(
  decide({ proposal: refinement({ deltaBaseM: [0, 0, 0], rotationDeltaRad: [0, 0, 0] }) }),
...
assert.deepEqual(
  decide({ proposal: refinement({ rotationDeltaRad: [0, 0, limits.maxRotationRad + 1e-6] }) }),
  { approved: false, reason: 'refinement_rotation_exceeds_limit' }
);
const rotatedRefinement = decide({
  proposal: refinement({ rotationDeltaRad: [0, limits.maxRotationRad, 0] }),
});
assert.equal(rotatedRefinement.approved, true);
assert.deepEqual(rotatedRefinement.command.rotation_delta_rad, [0, limits.maxRotationRad, 0]);
const rotationOnlyRefinement = decide({
  proposal: refinement({ deltaBaseM: [0, 0, 0], rotationDeltaRad: [0, limits.maxRotationRad, 0] }),
});
assert.equal(rotationOnlyRefinement.approved, true);
assert.deepEqual(rotationOnlyRefinement.command.delta_base_m, [0, 0, 0]);
assert.deepEqual(
  decide({ proposal: refinement(), session: makeSession({ refinementSteps: 6 }) }),
