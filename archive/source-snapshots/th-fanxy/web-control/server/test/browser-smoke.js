...
async function run() {
  const bundledPython = path.join(os.homedir(), 'miniconda3', 'bin', 'python');
  lumosServer = spawnChild('proxy.js', {
    STARTOUCH_SIMULATE: '1',
    STARTOUCH_PYTHON: process.env.STARTOUCH_TEST_PYTHON ||
  ...
  const finalState = await evaluate(`(() => ({
    ...
    cameraSource: document.getElementById('camera-feed')?.getAttribute('src') || '',
    xvisioHeading: [...document.querySelectorAll('.sec-header span')]
      .some(node => node.textContent.trim() === 'XVisio RGB-D 视觉'),
    xvisioOverlayLabel: document.body.textContent.includes('XVisio 识别与抓取叠加'),
    xvisioDepthLabel: document.body.textContent.includes('XVisio 深度热力图')
  }))()`);
  ...
  const checks = {
    ...
    xvisioCameraNamingAndRawStream:
      finalState.xvisioHeading &&
      finalState.xvisioOverlayLabel &&
      finalState.xvisioDepthLabel &&
      finalState.cameraSource.startsWith('/camera_xvisio_raw') &&
      !finalState.bodyText.includes('D435 视觉') &&
      !finalState.bodyText.includes('Lumos 鱼眼'),
