...
def test_online_config_has_exact_roles_local_models_and_latest_only_schedule():
    ...
    assert config.task_checkpoint_validated is True
    assert config.accepted_task_labels == ("bottle",)
    ...
    assert 0.65 <= config.identity_config.min_memory_confidence <= 0.70
    assert config.identity_config.min_memory_confidence > config.detector_min_score
    assert config.identity_config.max_actionable_position_std_m == 0.030
    assert config.identity_config.position_gate_max_age_ns == 1_500_000_000
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda raw: raw["online"].pop("task_labels"),
            "task_labels",
        ),
    ],
)
def test_online_config_rejects_unsafe_or_wrongly_typed_values(tmp_path, mutation, error):
    ...
    evidence = raw["online"].get("task_checkpoint_evidence")
    if evidence is not None:
        raw["online"]["task_checkpoint_evidence"] = str(
            (source.parent / evidence).resolve()
        )
    mutation(raw)
