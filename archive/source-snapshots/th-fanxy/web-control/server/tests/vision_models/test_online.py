...
from vision.grasp_geometry import GraspCandidateGeometry, GraspPreviewStatus
from vision.d435_instance_verifier import D435InstanceVerifier
...
def engine(
    ...
    return OnlinePerceptionEngine(
        ...
        d435_instance_verifier=d435_instance_verifier,
    )
...
class SyntheticGraspPreview:
    def evaluate(self, **kwargs):
        target = kwargs["targets"][0]
        candidate = GraspCandidateGeometry(
            identity_id=target.identity_id,
            detection_id=target.detection_id,
            source_stamp=FrameStamp("lumos_rgb+d435_depth", 1, 1_000_000_000),
            calibration_id="sha256:" + "a" * 64,
            evidence_ids=("sha256:" + "b" * 64,),
            grasp_xyz_m=[0.4, 0.0, 0.08],
            pregrasp_xyz_m=[0.4, 0.0, 0.16],
            retreat_xyz_m=[0.4, 0.0, 0.18],
            yaw_rad=0.2,
            width_m=0.04,
            object_height_m=0.08,
            valid_points=120,
            central_fraction=0.92,
            axis_mad_m=[0.002, 0.002, 0.003],
            grasp_lumos_px=(2, 2),
            grasp_d435_px=(2.1, 2.2),
        )
        return (
            GraspPreviewStatus(
                preview_id="sha256:" + "d" * 64,
                candidate=candidate,
                grasp_xyz_m=candidate.grasp_xyz_m,
                stable_samples=5,
                allowed=True,
                blockers=(),
            ),
...
def test_grasp_preview_is_attached_to_exact_target_and_execution_stays_locked():
    result = engine(grasp_preview=SyntheticGraspPreview()).process(
        pair(),
        robot_pose=None,
        calibration=None,
        arm_stationary=True,
    ...
    target = result.targets[0]
    preview = result.to_event()["targets"][0]["grasp_preview"]

    assert target.grasp_preview is not None
    assert preview["identity_id"] == target.identity_id
    assert preview["grasp_lumos_px"] == [2, 2]
    assert preview["allowed"] is False
    assert "task_checkpoint_unvalidated" in preview["blockers"]
    overlay = render_overlay(pair().rgb.image_rgb, result)
    assert np.count_nonzero(overlay) > 0


def test_configured_d435_same_instance_gate_fails_closed_without_d435_rgb() -> None:
    result = engine(
        grasp_preview=SyntheticGraspPreview(),
        d435_instance_verifier=D435InstanceVerifier(),
    ).process(
        pair(),
        robot_pose=None,
        calibration=None,
    ...
    target = result.targets[0]
    event_target = result.to_event()["targets"][0]
    assert target.d435_verification is not None
    assert target.d435_verification.verified is False
    assert "d435_rgb_unavailable" in target.d435_verification.blockers
    assert target.actionable is False
    assert target.grasp_preview is not None
    assert target.grasp_preview.allowed is False
    assert "d435_rgb_unavailable" in target.grasp_preview.blockers
    assert event_target["d435_same_instance"]["verified"] is False
...
def test_online_engine_forwards_verified_stationarity_and_calibration() -> None:
    ...
    engine(active_view=recorder).process(
        ...
        arm_stationary=True,
        now_ns=1_020_000_000,
        refinement_evidence_ids=("sha256:" + "e" * 64,),
    )
    ...
    assert recorder.kwargs["refinement_evidence_ids"] == ("sha256:" + "e" * 64,)
