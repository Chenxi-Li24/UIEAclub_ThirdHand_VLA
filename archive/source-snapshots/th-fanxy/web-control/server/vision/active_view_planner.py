...
ROTATION_PROPOSAL_SAFETY_FACTOR = 0.98
...
def evaluate_depth_quality(
    ...
    if np.any(projected):
        selected_d435 = selected_d435[projected]
        selected_lumos = selected_lumos[projected]
        ...
        if instance_mask is not None:
            ...
            selected_d435 = selected_d435[supported]
            selected_lumos = selected_lumos[supported]
            uv_d435 = uv_d435[supported]
    else:
        selected_d435 = np.empty((0, 3), dtype=float)
        selected_lumos = np.empty((0, 3), dtype=float)
        uv_d435 = np.empty((0, 2), dtype=float)

    valid_points = len(selected_d435)
    if valid_points:
        points_d435 = selected_d435
        points_base = transform_points(t_base, selected_lumos)
        pixels = uv_d435
...
def propose_refinement(
    ...
    try:
        ...
        max_rotation_rad = float(config.max_rotation_rad)
        ttl_ns = config.proposal_ttl_ns
    except (AttributeError, TypeError, ValueError) as exc:
    ...
    if max_steps > 6:
        raise InvalidDataError("view refinement cannot exceed six steps")
    if max_translation_m * max_steps > 0.060 + 1e-12:
        raise InvalidDataError("view refinement total translation cannot exceed 60 mm")
    if max_rotation_rad * max_steps > np.deg2rad(15.0) + 1e-12:
        raise InvalidDataError("view refinement total rotation cannot exceed 15 degrees")
    ...
    if not np.isfinite(max_rotation_rad) or not 0.0 < max_rotation_rad <= np.deg2rad(5.0):
        raise InvalidDataError("view refinement rotation limit must be within 5 degrees")
    ...
    table_normal_base = config.table_plane.normal_base
    tabletop_lateral_axis = np.cross(table_normal_base, optical_axis_base)
    tabletop_axis_norm = float(np.linalg.norm(tabletop_lateral_axis))
    if tabletop_axis_norm <= 1e-12:
        tabletop_delta_base = raw_delta_base - table_normal_base * np.dot(
            raw_delta_base,
            table_normal_base,
        )
    else:
        tabletop_lateral_axis /= tabletop_axis_norm
        tabletop_delta_base = tabletop_lateral_axis * np.dot(
            raw_delta_base,
            tabletop_lateral_axis,
        )
    norm = float(np.linalg.norm(tabletop_delta_base))
    delta_base = (
        np.zeros(3, dtype=float)
        if norm <= 1e-12
        else tabletop_delta_base * min(1.0, max_translation_m / norm)
    )
    target_ray_d435 = quality.center_d435_m / np.linalg.norm(quality.center_d435_m)
    optical_axis_d435 = np.array([0.0, 0.0, 1.0], dtype=float)
    rotation_axis_d435 = np.cross(optical_axis_d435, target_ray_d435)
    axis_norm = float(np.linalg.norm(rotation_axis_d435))
    if axis_norm <= 1e-12:
        rotation_delta_base = np.zeros(3, dtype=float)
    else:
        rotation_angle = min(
            float(
                np.arctan2(
                    axis_norm,
                    np.dot(optical_axis_d435, target_ray_d435),
                )
            ),
            max_rotation_rad * ROTATION_PROPOSAL_SAFETY_FACTOR,
        )
        rotation_delta_d435 = rotation_axis_d435 * (rotation_angle / axis_norm)
        rotation_delta_base = transform[:3, :3] @ rotation_delta_d435
    if norm <= 1e-12 and float(np.linalg.norm(rotation_delta_base)) <= 1e-12:
    ...
    return ObservationMoveProposal.refine(
        ...
        rotation_delta_rad=rotation_delta_base,
