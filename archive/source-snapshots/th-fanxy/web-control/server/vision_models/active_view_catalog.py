...
@dataclass(frozen=True)
class ActiveViewFoundationGuard:
    """Hold immutable camera/table evidence independently of motion poses."""
    ...
    def verify_unchanged(self) -> bool:
        try:
            current = load_active_view_foundation(
                self.camera_path,
                self.table_path,
            ...
            return False
        return (
            current.camera_source_id == self.evidence.camera_source_id
            and current.table_source_id == self.evidence.table_source_id
        )
