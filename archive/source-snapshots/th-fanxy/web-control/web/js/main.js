...
class UIControls {
  ...
  build() {
    ...
    if (window.ThirdHandGroundingUI) {
      this.grounding = window.ThirdHandGroundingUI.createGroundingUI({
        root: document,
        send: command => this.ws.send(command),
      });
      window.addEventListener('beforeunload', () => this.grounding?.destroy(), { once: true });
    }
    ...
    document.getElementById('btn-disconnect').addEventListener('click', () => {
      ...
      this.grounding?.invalidate('机械臂连接已断开');
    ...
    document.getElementById('btn-estop-top').addEventListener('click', () => {
      ...
      this.grounding?.invalidate('软件停止已触发');
    ...
    // WebSocket 事件监听
    this.ws.on('ws_connection', data => {
      if (!data.connected) this.grounding?.invalidate('WebSocket 连接已断开');
    });
    ...
    this.ws.on('software_stop', data => {
      ...
      this.grounding?.invalidate('软件停止状态已更新');
    ...
    // ── XVisio RGB-D vision handlers ──────────────────────
    const cameraFeed = document.getElementById('camera-feed');
    const lumosFeed = document.getElementById('lumos-feed');
    const depthFeed = document.getElementById('depth-feed');
    const streamLeaseMs = 8000;
    let cameraFeedLoaded = false;
    let cameraRetryTimer = null;
    let cameraLeaseTimer = null;
    let cameraRetryCount = 0;
    let lumosRetryTimer = null;
    let lumosLeaseTimer = null;
    let lumosRetryCount = 0;
    let depthRetryTimer = null;
    let depthLeaseTimer = null;
    let depthRetryCount = 0;
    const lumosStreamUrl = '/camera_lumos_vision';
    ...
    const reloadCameraFeed = (resetBackoff = false) => {
      ...
      if (cameraLeaseTimer) clearTimeout(cameraLeaseTimer);
      cameraRetryTimer = null;
      cameraLeaseTimer = null;
      cameraFeedLoaded = false;
      cameraFeed.src = `/camera_xvisio_raw?stream=${Date.now()}`;
      cameraLeaseTimer = setTimeout(() => reloadCameraFeed(true), streamLeaseMs);
    };
    ...
    const reloadLumosFeed = (resetBackoff = false) => {
      ...
      if (lumosLeaseTimer) clearTimeout(lumosLeaseTimer);
      lumosRetryTimer = null;
      lumosLeaseTimer = null;
      lumosFeed.src = `${lumosStreamUrl}?stream=${Date.now()}`;
      lumosLeaseTimer = setTimeout(() => reloadLumosFeed(true), streamLeaseMs + 1500);
    };

    if (lumosFeed) {
      lumosFeed.addEventListener('load', () => {
        lumosRetryCount = 0;
        lumosFeed.title = 'XVisio algorithm stream connected';
        if (lumosRetryTimer) clearTimeout(lumosRetryTimer);
        lumosRetryTimer = null;
      });
      lumosFeed.addEventListener('error', () => {
        lumosFeed.title = 'XVisio algorithm stream reconnecting';
        if (lumosLeaseTimer) clearTimeout(lumosLeaseTimer);
        lumosLeaseTimer = null;
    ...
    const reloadDepthFeed = (resetBackoff = false) => {
      if (!depthFeed) return;
      if (resetBackoff) depthRetryCount = 0;
      if (depthRetryTimer) clearTimeout(depthRetryTimer);
      if (depthLeaseTimer) clearTimeout(depthLeaseTimer);
      depthRetryTimer = null;
      depthLeaseTimer = null;
      depthFeed.src = `/camera_depth?stream=${Date.now()}`;
      depthLeaseTimer = setTimeout(() => reloadDepthFeed(true), streamLeaseMs + 3000);
    };

    if (depthFeed) {
      depthFeed.addEventListener('load', () => {
        depthRetryCount = 0;
        depthFeed.title = 'XVisio depth stream connected';
        if (depthRetryTimer) clearTimeout(depthRetryTimer);
        depthRetryTimer = null;
      });
      depthFeed.addEventListener('error', () => {
        depthFeed.title = 'XVisio depth stream reconnecting';
        if (depthLeaseTimer) clearTimeout(depthLeaseTimer);
        depthLeaseTimer = null;
        if (depthRetryTimer) return;
        const delayMs = Math.min(1000 * (2 ** depthRetryCount), 10000);
        depthRetryCount += 1;
        depthRetryTimer = setTimeout(() => {
          depthRetryTimer = null;
          reloadDepthFeed();
        }, delayMs);
      });
      reloadDepthFeed(true);
    }
    ...
    this.ws.on('detection_result', (data) => {
      ...
      if (!list) {
        this.grounding?.handleDetections(targets);
        return;
      }
      list.replaceChildren();
      for (const obj of targets) {
        const identityId = Number(obj.identity_id ?? obj.identityId ?? obj.id);
        const item = document.createElement('div');
        item.className = 'detection-item';
        if (Number.isSafeInteger(identityId) && identityId >= 0) {
          item.dataset.groundingIdentityId = String(identityId);
        }
        const label = document.createElement('span');
        label.className = 'det-label';
        label.textContent = Number.isSafeInteger(identityId)
          ? `ID ${identityId} ${obj.label || 'unknown'}`
          : (obj.label || 'unknown');
        const confidence = document.createElement('span');
        confidence.className = 'det-conf';
        const score = Number(obj.score ?? obj.conf);
        confidence.textContent = Number.isFinite(score) ? `${(score * 100).toFixed(0)}%` : '--';
        const coordinates = document.createElement('div');
        coordinates.className = 'det-coords';
        const position = obj.pose?.xyz_m ?? obj.position_m;
        coordinates.textContent = Array.isArray(position)
          ? `base (${position.map(value => Number(value).toFixed(3)).join(', ')})m`
        ...
        const previewId = obj.grasp_preview?.preview_id;
        const actionable = obj.grasp_preview?.allowed === true &&
          Number.isSafeInteger(identityId) && typeof previewId === 'string';
        btn.textContent = actionable ? `Grasp ID ${identityId}` : 'Safety locked';
        btn.disabled = !actionable;
        btn.title = actionable
          ? `Grasp confirmed target ID ${identityId}`
          : (obj.grasp_preview?.blockers || obj.reasons || ['target_not_actionable']).join(', ');
        if (actionable) {
          btn.addEventListener('click', () => {
            this.ws.send({
              cmd: 'grasp_object', identity_id: identityId, preview_id: previewId,
            });
            this._log(`→ Grasp object ID ${identityId}`);
          });
        }
    ...
    const applyCameraStatus = (data) => {
      if (data.d435_ready === false) this.grounding?.invalidate('XVisio 视觉不可用');
      const dot = document.getElementById('cam-dot');
      const label = document.getElementById('cam-label');
      if (dot) dot.className = 'cam-dot ' + (data.d435_ready ? 'connected' : '');
      if (label) {
        label.textContent = 'XVisio: ' + (data.d435_ready
    ...
    this.ws.on('camera_error', (data) => {
      this.grounding?.invalidate('相机错误');
      this._log('⚠ Camera: ' + (data.msg || data.message || 'error'));
    });
    this.ws.on('vision_error', () => this.grounding?.invalidate('视觉模型错误'));
    this.ws.on('grounding_state', data => this.grounding?.handleState(data));
