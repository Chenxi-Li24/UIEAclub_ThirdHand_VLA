...
class GraspExecutionController {
  ...
  begin(message) {
    ...
    const target = this.getTarget(message.identityId);
    ...
    this.session = {
      sessionId,
      plan: freezePlan(decision.plan),
      euler: Object.freeze([...robot.tcpEulerRad]),
      phase: 'open',
    };
    try {
  ...
  _phaseCommand(phase, requestId) {
    ...
    const point = phase === 'pregrasp' ? plan.pregraspPointM
      : phase === 'descend' ? plan.graspPointM : plan.retreatPointM;
    return { cmd: 'move_l', position: [...point], euler: [...euler],
  ...
  onRobotEvent(event) {
    ...
    if (this.inFlight.phase === 'open' && event.reached !== true) {
    ...
    this.inFlight = null;
    if (completed.phase === 'recovery_retreat') {
      return this._abort(this.session.failureReason || 'grasp_recovery');
    }
    const next = {
      open: 'pregrasp', pregrasp: 'descend', descend: 'close', close: 'retreat',
    }[completed.phase];
    if (next) {
  ...
  _recoverRetreat(reason) {
    ...
    if (!this._sendPhase('recovery_retreat')) {
      return { handled: true, accepted: false, reason: 'robot_transport_unavailable' };
    }
    return { handled: true, status: 'recovery_retreat', reason };
  }
