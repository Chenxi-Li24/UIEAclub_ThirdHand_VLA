...
class LumosCapture:
    def __init__(
        ...
        self.raw_shape: tuple[int, ...] | None = None
    ...
    def wait_for_frame(self, after_sequence: int, timeout: float) -> tuple[bytes | None, int]:
    ...
    def wait_for_raw_snapshot(
        ...
        with self.condition:
            ...
            monotonic_ns = self.last_frame_monotonic_ns
        if frame is None or monotonic_ns <= 0:
            return None, sequence, monotonic_ns
        encoded, jpeg = cv2.imencode(
            ".jpg",
            frame,
            [cv2.IMWRITE_JPEG_QUALITY, max(95, self.jpeg_quality)],
        )
        if not encoded:
            self._publish_error("raw JPEG encoding failed")
            return None, sequence, monotonic_ns
        return jpeg.tobytes(), sequence, monotonic_ns
        ...
        def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
            ...
            if path == "/frame_raw.jpg":
                ...
                try:
                    ...
                    return
                if after_sequence < -1:
                    self.send_error(400, "after must be at least -1")
                    return
                jpeg, sequence, monotonic_ns = capture.wait_for_raw_snapshot(
                    after_sequence,
                    timeout=5.0,
                )
