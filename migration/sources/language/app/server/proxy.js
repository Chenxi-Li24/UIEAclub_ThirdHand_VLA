...
const { LanguageUpstreamBridge } = require('./language-upstream-bridge');
...
const { SkillExecutorRegistry } = require('./skill-executor-registry');
...
const asrV2RuntimePolicy = loadAsrV2RuntimePolicy(process.env);

const app = express();
app.use(express.static(path.join(__dirname, '..', 'web')));
app.get('/api/runtime-config', (_req, res) => {
  res.json({
    voice: { endpoint: config.language.voiceEndpoint },
    language: {
      ...languageOrchestrator.runtimeConfig(),
      directional: directionalOrchestrator.runtimeConfig(),
      voiceEndpoint: config.language.voiceEndpoint,
      executionBackend: languageUpstream ? 'formal-3000-upstream' : 'local-bridge',
      manualControlBackend: manualUpstreamEnabled ? 'formal-3000-upstream' : 'local-bridge',
      upstream: languageUpstream?.publicStatus() || null,
    },
  });
});
...
const languageUpstream = config.language.upstreamWs
  ? new LanguageUpstreamBridge({
    endpoint: config.language.upstreamWs,
    stateMaxAgeMs: config.language.stateMaxAgeMs,
    maxDeltaDeg: config.language.maxDeltaDeg,
    maxSpeedScale: config.language.speedScale,
  })
  : null;
const manualUpstreamEnabled = Boolean(
  languageUpstream && config.language.manualUpstreamControl
);
...
graspController = new GraspExecutionController({
  ...
  getTarget: identityId => latestVisionTargets.get(identityId),
...
function requestSoftwareStop() {
  if (languageUpstream) return languageUpstream.softwareStop();
  ...
  return bridge.softwareStop();
}

const skillExecutors = new SkillExecutorRegistry();
const languageCandidateOwners = new Map();
const languageOrchestrator = new ManualJointOrchestrator({
  enabled: config.language.realControlEnabled,
  jointLimitsDeg: config.jointLimitsDeg,
  maxDeltaDeg: config.language.maxDeltaDeg,
  speedScale: config.language.speedScale,
  stateMaxAgeMs: config.language.stateMaxAgeMs,
  jointToleranceDeg: config.language.jointToleranceDeg,
  gripperOpenTarget: config.language.gripperOpenTarget,
  gripperCloseTarget: config.language.gripperCloseTarget,
  gripperTimeoutMs: config.language.gripperTimeoutMs,
  moveTimeFor,
  getHomeTarget: () => languageUpstream
    ? languageUpstream.getPreset('home')
    : [...config.presets.home],
  sendRobot: command => {
    if (languageUpstream) return languageUpstream.send(command);
    if (command.cmd === 'preset_home') {
      const target = config.presets.home;
      return bridge.send({
        cmd: 'move_joint',
  ...
  softwareStop: requestSoftwareStop,
  getRobotState: () => languageUpstream?.getRobotState() || ({
    simulated: config.robot.simulate,
    connected: bridge.connected,
    stateFresh: stateReady && latestRobotStateAtMs !== null &&
      Date.now() - latestRobotStateAtMs <= config.language.stateMaxAgeMs,
    ageMs: latestRobotStateAtMs === null ? null : Date.now() - latestRobotStateAtMs,
    stateName: latestRobotStateName,
    motionActive,
    activeViewActive: activeViewIsActive(),
    graspActive: graspController.active === true,
    jointsDeg: latestJointsDeg === null ? null : [...latestJointsDeg],
    gripperPosition: latestGripperPosition,
  }),
  skillExecutors,
  onMessage: (ws, message) => send(ws, message),
});

const directionalOrchestrator = new DirectionalJointOrchestrator({
...
if (languageUpstream) {
  languageUpstream.on('message', message => {
    languageOrchestrator.handleBridgeEvent(message);
    directionalOrchestrator.handleBridgeEvent(message);
  });
  let lastManualConnection = null;
  languageUpstream.on('status', status => {
    broadcast({ type: 'language_backend', ...status });
    if (!manualUpstreamEnabled) return;
    const connection = languageUpstream.manualConnectionInfo();
    const signature = JSON.stringify(connection);
    if (signature !== lastManualConnection) {
      lastManualConnection = signature;
      broadcast({ type: 'connection', ...connection });
    }
  });
  languageUpstream.on('upstream_message', message => {
    if (!manualUpstreamEnabled) return;
    if ([
      'robot_state',
      'motion_state',
  ...
  languageUpstream.on('log', message => {
...
bridge.on('connection', message => {
  ...
  latestRobotStateName = null;
  latestGripperPosition = null;
  ...
  if (!languageUpstream && !message.connected) {
    languageOrchestrator.handleBridgeEvent(message);
    directionalOrchestrator.handleBridgeEvent(message);
  }
...
app.get('/camera_d435_raw', (req, res) => {
  const stream = cameraBridge.getD435RawMjpegStream();
  if (!stream) {
    res.status(503).send('D435 raw calibration stream not ready');
    return;
  }
  res.setHeader('Content-Type', 'multipart/x-mixed-replace; boundary=frame');
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate');
...
app.get('/api/vision/status', (_req, res) => {
  ...
  res.json(createActiveVisionSnapshot({ ...activeVisionDependencies, nowMs: Date.now }));
});
...
bridge.on('robot_state', message => {
  ...
  latestRobotStateName = message.state;
  latestGripperPosition = message.gripper_position;
  ...
  if (!languageUpstream) {
    ...
    languageOrchestrator.handleBridgeEvent(languageStateEvent);
    directionalOrchestrator.handleBridgeEvent(languageStateEvent);
  }
});
...
bridge.on('software_stop_complete', message => {
  if (manualUpstreamEnabled) return;
  if (!languageUpstream) languageOrchestrator.handleBridgeEvent(message);
  broadcast({
    type: 'software_stop',
    complete: true,
    depowered: true,
...
wss.on('connection', ws => {
  ...
  send(ws, {
    ...
    model: config.model,
    connection: manualUpstreamEnabled
      ? languageUpstream.manualConnectionInfo()
      : bridge.getInfo(),
    motion: {
  ...
  if (languageUpstream) send(ws, { type: 'language_backend', ...languageUpstream.publicStatus() });
  if (manualUpstreamEnabled) {
    languageUpstream.sendManual({ cmd: 'status' });
  } else {
  ...
  ws.on('message', data => {
    try {
      ...
      if (message.type === 'skill.candidate') {
        ...
        let owners = languageCandidateOwners.get(ws);
        if (!owners) {
          owners = new Map();
          languageCandidateOwners.set(ws, owners);
        }
        ...
        if (candidate.skill === DIRECTIONAL_SKILL) {
          ...
          languageOrchestrator.register(ws, candidate);
        }
      } else if (false) { /* Removed keyword compatibility path.
        const candidate = removedCompatibilityParser(message.text, {
          source: message.source,
        });
        if (!candidate) {
          send(ws, {
            type: 'removed.compat.no_match',
            source: message.source === 'transcript' ? 'transcript' : 'text',
            message: '严格指令适配器未匹配；未生成动作候选。',
          });
        } else {
          languageOrchestrator.register(ws, candidate);
          send(ws, {
            type: 'removed.compat.candidate',
            candidate,
          });
        }
      */ } else if (message.type === 'confirmation.decision') {
        const owners = languageCandidateOwners.get(ws);
        const owner = owners?.get(message.candidateId);
        if (owner === DIRECTIONAL_SKILL) {
          directionalOrchestrator.decide(ws, message);
        } else {
          languageOrchestrator.decide(ws, message);
        }
        owners?.delete(message.candidateId);
      } else {
        handleBrowserCommand(message, ws);
      }
    } catch (error) {
  ...
  ws.on('close', () => {
    languageOrchestrator.disconnect(ws);
    directionalOrchestrator.disconnect(ws);
    languageCandidateOwners.delete(ws);
...
function handleLegacyBrowserCommand(message, ws) {
  ...
  if (manualUpstreamEnabled && [
    ...
    if (!languageUpstream.sendManual(message)) {
      send(ws, { type: 'error', msg: '正式 3000 转发连接当前不可用' });
    }
    return;
  }
  switch (message.cmd) {
    ...
    case 'cancel_active_view': {
      ...
      if (parsed.command === 'start') {
        const startGate = authorizeActiveViewStart({
          ...
          activeViewActive: activeViewIsActive(),
          graspActive: graspController.active,
          motionActive,
        });
...
server.listen(config.web.port, config.web.host, () => {
  ...
  lumosService.start();
  if (asrV2RuntimePolicy.localCameraBridgeEnabled &&
      config.camera && config.camera.enabled !== false) {
    console.log('Camera:  D435 bridge enabled');
  }
  startLocalBridges(asrV2RuntimePolicy, {
    robot: bridge,
    camera: config.camera && config.camera.enabled !== false ? cameraBridge : null,
  });
  languageUpstream?.start();
});
