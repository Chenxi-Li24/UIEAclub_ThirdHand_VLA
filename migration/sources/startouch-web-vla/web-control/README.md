...
```text
浏览器
  -> 3000: Node.js server/proxy.js（唯一机械臂控制边界）
  -> JSON Lines
  -> LumosTouch Python + startouch_sdk
  -> SocketCAN can0
  -> Startouch 机械臂

3100: XVisio RGB-D / 目标识别（模拟机械臂，不访问 can0）
  -> 3000: 固定路径视频和识别事件转发
  -> 浏览器
```
...
先启动 3100 视觉服务，再启动 3000 真机控制服务。Ubuntu 的 systemd 部署分别使用
`thirdhand-web-control-vision-preview.service` 和
`thirdhand-web-control-main.service`。手动验证可使用：

```bash
# 终端 1：视觉服务，禁止访问真实机械臂
cd ~/arm/UIEAclub_ThirdHand_VLA/web-control/server
WEB_PORT=3100 CAMERA_ENABLED=1 STARTOUCH_SIMULATE=1 \
  XVISION_PROXY_ENABLED=0 npm start

# 终端 2：真实机械臂服务，只转发 3100 的视觉结果
cd ~/arm/UIEAclub_ThirdHand_VLA
...
网页右侧默认显示带检测框和选中目标的识别画面，也可切换到原始视频。目标选择只向
服务端发送左右序号（例如 `L1` 或 `R3`）；浏览器坐标永远不能直接用于机械臂运动。
分步和自动夹取均执行“悬停、下降、闭合、抬起”，结束后保持夹持，不自动回零或张开。
...
| `XVISION_PROXY_ENABLED` | `1` | 由 3000 转发 3100 的 XVisio 视频和事件 |
| `XVISION_SERVICE_URL` | `http://127.0.0.1:3100` | XVisio HTTP 服务 |
| `XVISION_WS_URL` | `ws://127.0.0.1:3100/ws` | XVisio 事件 WebSocket |
| `VISION_ROBOT_EXECUTION_ENABLED` | `0` | 视觉夹取总开关；默认锁定 |
...
视觉夹取还要求每一帧同时携带已批准的手眼标定、有效深度、稳定目标身份、静止机械臂
状态和服务端抓取预览。仅把 `VISION_ROBOT_EXECUTION_ENABLED` 改为 `1` 不会绕过这些
门禁。当前物理手眼验证状态为 `pending`，因此页面可以查看、切换和选择目标，但夹取按钮
应保持锁定。
