...
const xvisio = new VisionStatusStore({ staleAfterMs: 2000 });
xvisio.updateTargets({
  type: 'detection_result',
...
const xvisioCurrent = xvisio.snapshot(4100);
assert.equal(xvisioCurrent.online, true);
assert.equal(xvisioCurrent.modelReady, true);
assert.equal(xvisioCurrent.d435Ready, true);
assert.equal(xvisioCurrent.targets[0].identityStatus, 'confirmed');
assert.deepEqual(xvisioCurrent.roles, {
  canonicalRgb: 'xvisio_rgb',
  metricDepth: 'xvisio_depth',
  debugRgb: 'xvisio_rgb',
});
assert.deepEqual(xvisioCurrent.blockers, ['hardware_validation_pending']);
assert.equal(xvisioCurrent.robotExecutionEnabled, false);

const normalizedXvisio = new VisionStatusStore({ staleAfterMs: 2000 });
normalizedXvisio.updateTargets({
  type: 'detection_result',
  schema: 'thirdhand-va-detection-v2',
  ts: 4200,
  ...
  targets: [{
    ...
    rightOrdinal: 3,
    spatialLabel: 'L1/R3',
    position_m: [0.555, 0.029, 0.294],
    depth_m: 0.194,
    depthValidRatio: 0.58,
    graspWidthM: 0.062,
    previewIdShort: 'sha256:aaaaaaaaaaaa…',
    blockers: ['physical_grasp_execution_locked'],
    reasons: [],
    actionable: false,
  }],
});
const normalizedCurrent = normalizedXvisio.snapshot(4300);
assert.equal(normalizedCurrent.targets[0].spatialLabel, 'L1/R3');
assert.equal(normalizedCurrent.targets[0].selected, true);
assert.equal(normalizedCurrent.targets[0].leftOrdinal, 1);
assert.equal(normalizedCurrent.targets[0].rightOrdinal, 3);
assert.equal(normalizedCurrent.targets[0].depthValidRatio, 0.58);
assert.equal(normalizedCurrent.targets[0].graspWidthM, 0.062);
assert.equal(normalizedCurrent.targets[0].previewIdShort, 'sha256:aaaaaaaaaaaa…');
assert.deepEqual(normalizedCurrent.targets[0].graspReasons, [
  'physical_grasp_execution_locked',
]);
