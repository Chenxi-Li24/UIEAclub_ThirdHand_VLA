...
function sanitizeTarget(target) {
  ...
  const reportedIdentityStatus = target.identity_confirmed === true
    ? 'confirmed'
    : (target.identity_status ?? target.identityStatus);
  const identityStatus = new Set(['tentative', 'confirmed', 'occluded', 'inactive', 'ambiguous'])
    .has(reportedIdentityStatus) ? reportedIdentityStatus : 'unknown';
  ...
  const explicitBlockers = boundedStrings(target.blockers);
  const graspReasons = [];
  if (target.actionable !== true) {
    graspReasons.push(...(
      explicitBlockers.length
        ? explicitBlockers
        : (perceptionReasons.length ? perceptionReasons : ['perception_not_actionable'])
    ));
  }
  if (graspPreview === null && explicitBlockers.length === 0) {
    graspReasons.push('grasp_preview_unavailable');
  }
  graspReasons.push('physical_grasp_execution_locked');
  const depthValidRatio = finiteOrNull(target.depthValidRatio);
  const graspWidthM = finiteOrNull(target.graspWidthM);
  ...
  return {
    detectionId: nonNegativeIntegerOrNull(target.detection_id ?? target.detectionId),
    identityId: nonNegativeIntegerOrNull(target.identity_id ?? target.identityId ?? target.id),
    identityStatus,
    ...
    leftOrdinal: nonNegativeIntegerOrNull(target.left_ordinal ?? target.leftOrdinal),
    rightOrdinal: nonNegativeIntegerOrNull(target.right_ordinal ?? target.rightOrdinal),
    spatialLabel: boundedString(target.spatialLabel, 32),
    depthValidRatio: depthValidRatio !== null && depthValidRatio >= 0 && depthValidRatio <= 1
      ? depthValidRatio
      : null,
    graspWidthM: graspWidthM !== null && graspWidthM > 0 ? graspWidthM : null,
    previewIdShort: boundedString(target.previewIdShort, 32),
    targetState,
...
class VisionStatusStore {
  ...
  updateTargets(event) {
    ...
    this._timestamp(event);
    if (event.schema === 'thirdhand-va-detection-v2') {
      this.online = true;
      ...
      this.blockers = (event.hardware_validation ?? event.hardwareValidation) === 'pending'
