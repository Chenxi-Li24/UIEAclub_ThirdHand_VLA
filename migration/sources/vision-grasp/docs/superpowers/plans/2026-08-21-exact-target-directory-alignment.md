...
**Goal:** Align `PinZiZhuaQuSkill` with the approved target tree while ensuring every newly listed module has a real, independently testable responsibility rather than being an empty placeholder.
...
- Work stays inside `PinZiZhuaQuSkill`; no commit is created because the whole subtree is untracked in its parent repository.
