...
class FakeClaude:
    ...
    def chat(self, text: str) -> dict[str, Any]:
        ...
        return {
            "text": "好的，我已理解你的指令。",
            "actions": [{"tool": "open_gripper", "input": {}}],
        }
...
class VoiceBridgeProtocolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_round_trip_and_event_order(self) -> None:
        ...
        self.assertEqual(candidate["tool"], "open_gripper")
        ...
        self.assertEqual(candidate["payload"]["params"]["action"], "gripper.open")
        self.assertIn("traceId", candidate)
...
class VoiceBridgeSafetyTests(unittest.TestCase):
    def test_bridge_keeps_claude_but_does_not_restore_whisper_or_robot(self) -> None:
        ...
        self.assertEqual(imported_names, ["ClaudeAgent"])
        self.assertNotIn("WhisperASR", source)
