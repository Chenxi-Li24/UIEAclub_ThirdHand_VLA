...
function validateRuntimeAuthorization(env = process.env) {
  const localCameraEnabled = env.THIRDHAND_VA_ENABLE_CAMERA === '1';
  const externalVisionEnabled = typeof env.THIRDHAND_VA_VISION_WS_URL === 'string'
    && env.THIRDHAND_VA_VISION_WS_URL.length > 0;
  const cameraEnabled = localCameraEnabled || externalVisionEnabled;
  const robotEnabled = env.THIRDHAND_VA_ENABLE_ROBOT === '1';
  if (localCameraEnabled && env.THIRDHAND_VA_ALLOW_CAMERA !== '1') {
...
async function main() {
  ...
  const cameraBridge = visionWsUrl
    ? new VisionServiceClient({
      ...
      wsUrl: visionWsUrl,
      httpUrl: process.env.THIRDHAND_VISION_HTTP_URL,
    })
    : new CameraBridge({ projectRoot });
  ...
  if (robotEnabled) {
    ...
    const robotDependencies = {
      ...
      ...(config.robot.backend === 'websocket'
        ? {
          WebSocketImpl: require('ws'),
          url: process.env.THIRDHAND_ROBOT_WS_URL,
        } : {}),
    };
